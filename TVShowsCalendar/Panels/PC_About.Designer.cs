namespace TVShowsCalendar.Panels
{
	partial class PC_About
	{
		/// <summary> 
		/// Required designer variable.
		/// </summary>
		private System.ComponentModel.IContainer components = null;

		/// <summary> 
		/// Clean up any resources being used.
		/// </summary>
		/// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
		protected override void Dispose(bool disposing)
		{
			if (disposing && (components != null))
			{
				components.Dispose();
			}
			base.Dispose(disposing);
		}

		#region Component Designer generated code

		/// <summary> 
		/// Required method for Designer support - do not modify 
		/// the contents of this method with the code editor.
		/// </summary>
		private void InitializeComponent()
		{
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(PC_About));
			this.TLP_Main = new System.Windows.Forms.TableLayoutPanel();
			this.P_OptionContainer = new System.Windows.Forms.Panel();
			this.verticalScroll1 = new SlickControls.Controls.VerticalScroll();
			this.P_Stuff = new System.Windows.Forms.Panel();
			this.P_Changelog = new System.Windows.Forms.Panel();
			this.panel1 = new System.Windows.Forms.Panel();
			this.B_ChangeLog = new SlickControls.Controls.SlickButton();
			this.P_Licence = new System.Windows.Forms.TableLayoutPanel();
			this.P_Tips = new System.Windows.Forms.TableLayoutPanel();
			this.autoSizeLabel1 = new TVShowsCalendar.UserControls.AutoSizeLabel();
			this.sectionPanel1 = new TVShowsCalendar.UserControls.SectionPanel();
			this.P_AppInfo = new System.Windows.Forms.TableLayoutPanel();
			this.B_Icons8 = new SlickControls.Controls.SlickButton();
			this.B_TMDb = new SlickControls.Controls.SlickButton();
			this.L_Icons8Info = new TVShowsCalendar.UserControls.AutoSizeLabel();
			this.L_ShowsCalendarInfo = new TVShowsCalendar.UserControls.AutoSizeLabel();
			this.SP_Icons8 = new TVShowsCalendar.UserControls.SectionPanel();
			this.SP_TMDb = new TVShowsCalendar.UserControls.SectionPanel();
			this.SP_ShowsCalendar = new TVShowsCalendar.UserControls.SectionPanel();
			this.L_TMDbInfo = new TVShowsCalendar.UserControls.AutoSizeLabel();
			this.P_Tiles = new System.Windows.Forms.Panel();
			this.T_Changelog = new SlickControls.Controls.SlickTile();
			this.T_Licence = new SlickControls.Controls.SlickTile();
			this.T_Tips = new SlickControls.Controls.SlickTile();
			this.T_AppInfo = new SlickControls.Controls.SlickTile();
			this.P_Spacer = new SlickControls.Controls.DBPanel();
			this.P_Spacer_2 = new SlickControls.Controls.DBPanel();
			this.Content = new System.Windows.Forms.FlowLayoutPanel();
			this.TLP_Main.SuspendLayout();
			this.P_OptionContainer.SuspendLayout();
			this.P_Stuff.SuspendLayout();
			this.P_Changelog.SuspendLayout();
			this.panel1.SuspendLayout();
			((System.ComponentModel.ISupportInitialize)(this.B_ChangeLog)).BeginInit();
			this.P_Tips.SuspendLayout();
			this.P_AppInfo.SuspendLayout();
			((System.ComponentModel.ISupportInitialize)(this.B_Icons8)).BeginInit();
			((System.ComponentModel.ISupportInitialize)(this.B_TMDb)).BeginInit();
			this.P_Tiles.SuspendLayout();
			this.SuspendLayout();
			// 
			// TLP_Main
			// 
			this.TLP_Main.ColumnCount = 3;
			this.TLP_Main.ColumnStyles.Add(new System.Windows.Forms.ColumnStyle(System.Windows.Forms.SizeType.Absolute, 175F));
			this.TLP_Main.ColumnStyles.Add(new System.Windows.Forms.ColumnStyle(System.Windows.Forms.SizeType.Absolute, 1F));
			this.TLP_Main.ColumnStyles.Add(new System.Windows.Forms.ColumnStyle(System.Windows.Forms.SizeType.Percent, 100F));
			this.TLP_Main.Controls.Add(this.P_OptionContainer, 2, 1);
			this.TLP_Main.Controls.Add(this.P_Tiles, 0, 1);
			this.TLP_Main.Controls.Add(this.P_Spacer, 1, 1);
			this.TLP_Main.Controls.Add(this.P_Spacer_2, 0, 0);
			this.TLP_Main.Dock = System.Windows.Forms.DockStyle.Fill;
			this.TLP_Main.Location = new System.Drawing.Point(0, 30);
			this.TLP_Main.Margin = new System.Windows.Forms.Padding(2, 3, 2, 3);
			this.TLP_Main.Name = "TLP_Main";
			this.TLP_Main.RowCount = 2;
			this.TLP_Main.RowStyles.Add(new System.Windows.Forms.RowStyle(System.Windows.Forms.SizeType.Absolute, 1F));
			this.TLP_Main.RowStyles.Add(new System.Windows.Forms.RowStyle(System.Windows.Forms.SizeType.Percent, 100F));
			this.TLP_Main.RowStyles.Add(new System.Windows.Forms.RowStyle(System.Windows.Forms.SizeType.Absolute, 40F));
			this.TLP_Main.Size = new System.Drawing.Size(783, 1408);
			this.TLP_Main.TabIndex = 13;
			// 
			// P_OptionContainer
			// 
			this.P_OptionContainer.Controls.Add(this.verticalScroll1);
			this.P_OptionContainer.Controls.Add(this.P_Stuff);
			this.P_OptionContainer.Dock = System.Windows.Forms.DockStyle.Fill;
			this.P_OptionContainer.Location = new System.Drawing.Point(176, 1);
			this.P_OptionContainer.Margin = new System.Windows.Forms.Padding(0);
			this.P_OptionContainer.Name = "P_OptionContainer";
			this.P_OptionContainer.Size = new System.Drawing.Size(607, 1407);
			this.P_OptionContainer.TabIndex = 34;
			// 
			// verticalScroll1
			// 
			this.verticalScroll1.BarColor = null;
			this.verticalScroll1.Dock = System.Windows.Forms.DockStyle.Right;
			this.verticalScroll1.LinkedControl = this.P_Stuff;
			this.verticalScroll1.Location = new System.Drawing.Point(603, 0);
			this.verticalScroll1.Margin = new System.Windows.Forms.Padding(0, 0, 1, 0);
			this.verticalScroll1.Name = "verticalScroll1";
			this.verticalScroll1.Size = new System.Drawing.Size(12, 281);
			this.verticalScroll1.TabIndex = 35;
			// 
			// P_Stuff
			// 
			this.P_Stuff.AutoSize = true;
			this.P_Stuff.AutoSizeMode = System.Windows.Forms.AutoSizeMode.GrowAndShrink;
			this.P_Stuff.Controls.Add(this.P_Changelog);
			this.P_Stuff.Controls.Add(this.P_Licence);
			this.P_Stuff.Controls.Add(this.P_Tips);
			this.P_Stuff.Controls.Add(this.P_AppInfo);
			this.P_Stuff.Location = new System.Drawing.Point(0, 0);
			this.P_Stuff.MinimumSize = new System.Drawing.Size(600, 200);
			this.P_Stuff.Name = "P_Stuff";
			this.P_Stuff.Size = new System.Drawing.Size(600, 798);
			this.P_Stuff.TabIndex = 34;
			// 
			// P_Changelog
			// 
			this.P_Changelog.AutoSize = true;
			this.P_Changelog.AutoSizeMode = System.Windows.Forms.AutoSizeMode.GrowAndShrink;
			this.P_Changelog.Controls.Add(this.panel1);
			this.P_Changelog.Dock = System.Windows.Forms.DockStyle.Top;
			this.P_Changelog.Location = new System.Drawing.Point(0, 758);
			this.P_Changelog.Name = "P_Changelog";
			this.P_Changelog.Size = new System.Drawing.Size(600, 40);
			this.P_Changelog.TabIndex = 3;
			this.P_Changelog.Visible = false;
			// 
			// panel1
			// 
			this.panel1.Controls.Add(this.B_ChangeLog);
			this.panel1.Dock = System.Windows.Forms.DockStyle.Bottom;
			this.panel1.Location = new System.Drawing.Point(0, 0);
			this.panel1.Name = "panel1";
			this.panel1.Size = new System.Drawing.Size(600, 40);
			this.panel1.TabIndex = 15;
			// 
			// B_ChangeLog
			// 
			this.B_ChangeLog.Anchor = System.Windows.Forms.AnchorStyles.Right;
			this.B_ChangeLog.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Dpi;
			this.B_ChangeLog.AutoSizeMode = System.Windows.Forms.AutoSizeMode.GrowAndShrink;
			this.B_ChangeLog.ColorShade = null;
			this.B_ChangeLog.Cursor = System.Windows.Forms.Cursors.Hand;
			this.B_ChangeLog.Font = new System.Drawing.Font("Nirmala UI", 9.75F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(0)));
			this.B_ChangeLog.ForeColor = System.Drawing.Color.FromArgb(((int)(((byte)(250)))), ((int)(((byte)(250)))), ((int)(((byte)(250)))));
			this.B_ChangeLog.IconSize = 16;
			this.B_ChangeLog.Image = global::TVShowsCalendar.Properties.Resources.Tiny_Log;
			this.B_ChangeLog.Location = new System.Drawing.Point(425, 5);
			this.B_ChangeLog.Margin = new System.Windows.Forms.Padding(0);
			this.B_ChangeLog.Name = "B_ChangeLog";
			this.B_ChangeLog.Padding = new System.Windows.Forms.Padding(10, 5, 4, 5);
			this.B_ChangeLog.Size = new System.Drawing.Size(160, 30);
			this.B_ChangeLog.TabIndex = 15;
			this.B_ChangeLog.Text = "FULL CHANGE-LOG";
			this.B_ChangeLog.Click += new System.EventHandler(this.B_ChangeLog_Click);
			// 
			// P_Licence
			// 
			this.P_Licence.ColumnStyles.Add(new System.Windows.Forms.ColumnStyle(System.Windows.Forms.SizeType.Absolute, 20F));
			this.P_Licence.Dock = System.Windows.Forms.DockStyle.Top;
			this.P_Licence.Location = new System.Drawing.Point(0, 623);
			this.P_Licence.Name = "P_Licence";
			this.P_Licence.Size = new System.Drawing.Size(600, 135);
			this.P_Licence.TabIndex = 2;
			this.P_Licence.Visible = false;
			// 
			// P_Tips
			// 
			this.P_Tips.AutoSize = true;
			this.P_Tips.AutoSizeMode = System.Windows.Forms.AutoSizeMode.GrowAndShrink;
			this.P_Tips.ColumnStyles.Add(new System.Windows.Forms.ColumnStyle(System.Windows.Forms.SizeType.Percent, 100F));
			this.P_Tips.Controls.Add(this.autoSizeLabel1, 0, 1);
			this.P_Tips.Controls.Add(this.sectionPanel1, 0, 0);
			this.P_Tips.Dock = System.Windows.Forms.DockStyle.Top;
			this.P_Tips.Location = new System.Drawing.Point(0, 455);
			this.P_Tips.Name = "P_Tips";
			this.P_Tips.RowCount = 3;
			this.P_Tips.RowStyles.Add(new System.Windows.Forms.RowStyle());
			this.P_Tips.RowStyles.Add(new System.Windows.Forms.RowStyle());
			this.P_Tips.RowStyles.Add(new System.Windows.Forms.RowStyle(System.Windows.Forms.SizeType.Absolute, 20F));
			this.P_Tips.Size = new System.Drawing.Size(600, 168);
			this.P_Tips.TabIndex = 1;
			this.P_Tips.Visible = false;
			// 
			// autoSizeLabel1
			// 
			this.autoSizeLabel1.Dock = System.Windows.Forms.DockStyle.Top;
			this.autoSizeLabel1.Location = new System.Drawing.Point(3, 57);
			this.autoSizeLabel1.Name = "autoSizeLabel1";
			this.autoSizeLabel1.Size = new System.Drawing.Size(594, 88);
			this.autoSizeLabel1.TabIndex = 16;
			this.autoSizeLabel1.Text = resources.GetString("autoSizeLabel1.Text");
			// 
			// sectionPanel1
			// 
			this.sectionPanel1.Active = false;
			this.sectionPanel1.AutoHide = false;
			this.sectionPanel1.AutoSize = true;
			this.sectionPanel1.AutoSizeMode = System.Windows.Forms.AutoSizeMode.GrowAndShrink;
			this.sectionPanel1.Dock = System.Windows.Forms.DockStyle.Top;
			this.sectionPanel1.Flavor = null;
			this.sectionPanel1.Icon = global::TVShowsCalendar.Properties.Resources.Icon_22;
			this.sectionPanel1.Info = "";
			this.sectionPanel1.Location = new System.Drawing.Point(0, 0);
			this.sectionPanel1.Margin = new System.Windows.Forms.Padding(0);
			this.sectionPanel1.MaximumSize = new System.Drawing.Size(9999, 54);
			this.sectionPanel1.Name = "sectionPanel1";
			this.sectionPanel1.Size = new System.Drawing.Size(600, 54);
			this.sectionPanel1.TabIndex = 1;
			this.sectionPanel1.Text = "Shows Calendar";
			// 
			// P_AppInfo
			// 
			this.P_AppInfo.AutoSize = true;
			this.P_AppInfo.AutoSizeMode = System.Windows.Forms.AutoSizeMode.GrowAndShrink;
			this.P_AppInfo.ColumnStyles.Add(new System.Windows.Forms.ColumnStyle(System.Windows.Forms.SizeType.Percent, 100F));
			this.P_AppInfo.Controls.Add(this.B_Icons8, 0, 7);
			this.P_AppInfo.Controls.Add(this.B_TMDb, 0, 4);
			this.P_AppInfo.Controls.Add(this.L_Icons8Info, 0, 6);
			this.P_AppInfo.Controls.Add(this.L_ShowsCalendarInfo, 0, 1);
			this.P_AppInfo.Controls.Add(this.SP_Icons8, 0, 5);
			this.P_AppInfo.Controls.Add(this.SP_TMDb, 0, 2);
			this.P_AppInfo.Controls.Add(this.SP_ShowsCalendar, 0, 0);
			this.P_AppInfo.Controls.Add(this.L_TMDbInfo, 0, 3);
			this.P_AppInfo.Dock = System.Windows.Forms.DockStyle.Top;
			this.P_AppInfo.Location = new System.Drawing.Point(0, 0);
			this.P_AppInfo.Name = "P_AppInfo";
			this.P_AppInfo.RowCount = 8;
			this.P_AppInfo.RowStyles.Add(new System.Windows.Forms.RowStyle());
			this.P_AppInfo.RowStyles.Add(new System.Windows.Forms.RowStyle());
			this.P_AppInfo.RowStyles.Add(new System.Windows.Forms.RowStyle());
			this.P_AppInfo.RowStyles.Add(new System.Windows.Forms.RowStyle());
			this.P_AppInfo.RowStyles.Add(new System.Windows.Forms.RowStyle());
			this.P_AppInfo.RowStyles.Add(new System.Windows.Forms.RowStyle());
			this.P_AppInfo.RowStyles.Add(new System.Windows.Forms.RowStyle());
			this.P_AppInfo.RowStyles.Add(new System.Windows.Forms.RowStyle());
			this.P_AppInfo.Size = new System.Drawing.Size(600, 455);
			this.P_AppInfo.TabIndex = 0;
			// 
			// B_Icons8
			// 
			this.B_Icons8.Anchor = System.Windows.Forms.AnchorStyles.Right;
			this.B_Icons8.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Dpi;
			this.B_Icons8.AutoSizeMode = System.Windows.Forms.AutoSizeMode.GrowAndShrink;
			this.B_Icons8.ColorShade = null;
			this.B_Icons8.Cursor = System.Windows.Forms.Cursors.Hand;
			this.B_Icons8.Font = new System.Drawing.Font("Nirmala UI", 9.75F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(0)));
			this.B_Icons8.ForeColor = System.Drawing.Color.FromArgb(((int)(((byte)(250)))), ((int)(((byte)(250)))), ((int)(((byte)(250)))));
			this.B_Icons8.IconSize = 16;
			this.B_Icons8.Image = global::TVShowsCalendar.Properties.Resources.Tiny_RestoreWindow;
			this.B_Icons8.Location = new System.Drawing.Point(456, 425);
			this.B_Icons8.Margin = new System.Windows.Forms.Padding(0, 0, 15, 0);
			this.B_Icons8.Name = "B_Icons8";
			this.B_Icons8.Padding = new System.Windows.Forms.Padding(10, 5, 5, 5);
			this.B_Icons8.Size = new System.Drawing.Size(129, 30);
			this.B_Icons8.TabIndex = 18;
			this.B_Icons8.Text = "VISIT ICONS8";
			this.B_Icons8.Click += new System.EventHandler(this.B_Icons8_Click);
			// 
			// B_TMDb
			// 
			this.B_TMDb.Anchor = System.Windows.Forms.AnchorStyles.Right;
			this.B_TMDb.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Dpi;
			this.B_TMDb.AutoSizeMode = System.Windows.Forms.AutoSizeMode.GrowAndShrink;
			this.B_TMDb.ColorShade = null;
			this.B_TMDb.Cursor = System.Windows.Forms.Cursors.Hand;
			this.B_TMDb.Font = new System.Drawing.Font("Nirmala UI", 9.75F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(0)));
			this.B_TMDb.ForeColor = System.Drawing.Color.FromArgb(((int)(((byte)(250)))), ((int)(((byte)(250)))), ((int)(((byte)(250)))));
			this.B_TMDb.IconSize = 16;
			this.B_TMDb.Image = global::TVShowsCalendar.Properties.Resources.Tiny_RestoreWindow;
			this.B_TMDb.Location = new System.Drawing.Point(456, 284);
			this.B_TMDb.Margin = new System.Windows.Forms.Padding(0, 0, 15, 0);
			this.B_TMDb.Name = "B_TMDb";
			this.B_TMDb.Padding = new System.Windows.Forms.Padding(10, 5, 5, 5);
			this.B_TMDb.Size = new System.Drawing.Size(129, 30);
			this.B_TMDb.TabIndex = 17;
			this.B_TMDb.Text = "VISIT TMDb";
			this.B_TMDb.Click += new System.EventHandler(this.B_TMDb_Click);
			// 
			// L_Icons8Info
			// 
			this.L_Icons8Info.Dock = System.Windows.Forms.DockStyle.Top;
			this.L_Icons8Info.Location = new System.Drawing.Point(7, 371);
			this.L_Icons8Info.Margin = new System.Windows.Forms.Padding(7, 3, 7, 3);
			this.L_Icons8Info.Name = "L_Icons8Info";
			this.L_Icons8Info.Size = new System.Drawing.Size(586, 51);
			this.L_Icons8Info.TabIndex = 16;
			this.L_Icons8Info.Text = "Shows Calendar uses Icons8 for all its icon needs.\r\n\r\nIcons8 allows professional " +
    "and amateur designers to express their ideas by providing the right design eleme" +
    "nts at the right time.";
			// 
			// L_ShowsCalendarInfo
			// 
			this.L_ShowsCalendarInfo.Dock = System.Windows.Forms.DockStyle.Top;
			this.L_ShowsCalendarInfo.Location = new System.Drawing.Point(7, 57);
			this.L_ShowsCalendarInfo.Margin = new System.Windows.Forms.Padding(7, 3, 7, 3);
			this.L_ShowsCalendarInfo.Name = "L_ShowsCalendarInfo";
			this.L_ShowsCalendarInfo.Size = new System.Drawing.Size(586, 88);
			this.L_ShowsCalendarInfo.TabIndex = 15;
			this.L_ShowsCalendarInfo.Text = resources.GetString("L_ShowsCalendarInfo.Text");
			// 
			// SP_Icons8
			// 
			this.SP_Icons8.Active = false;
			this.SP_Icons8.AutoHide = false;
			this.SP_Icons8.AutoSize = true;
			this.SP_Icons8.AutoSizeMode = System.Windows.Forms.AutoSizeMode.GrowAndShrink;
			this.SP_Icons8.Dock = System.Windows.Forms.DockStyle.Top;
			this.SP_Icons8.Flavor = null;
			this.SP_Icons8.Icon = ((System.Drawing.Image)(resources.GetObject("SP_Icons8.Icon")));
			this.SP_Icons8.Info = "";
			this.SP_Icons8.Location = new System.Drawing.Point(0, 314);
			this.SP_Icons8.Margin = new System.Windows.Forms.Padding(0);
			this.SP_Icons8.MaximumSize = new System.Drawing.Size(9999, 54);
			this.SP_Icons8.Name = "SP_Icons8";
			this.SP_Icons8.Size = new System.Drawing.Size(600, 54);
			this.SP_Icons8.TabIndex = 2;
			this.SP_Icons8.Text = "Icons8";
			// 
			// SP_TMDb
			// 
			this.SP_TMDb.Active = false;
			this.SP_TMDb.AutoHide = false;
			this.SP_TMDb.AutoSize = true;
			this.SP_TMDb.AutoSizeMode = System.Windows.Forms.AutoSizeMode.GrowAndShrink;
			this.SP_TMDb.Dock = System.Windows.Forms.DockStyle.Top;
			this.SP_TMDb.Flavor = null;
			this.SP_TMDb.Icon = global::TVShowsCalendar.Properties.Resources.TMDB;
			this.SP_TMDb.Info = "";
			this.SP_TMDb.Location = new System.Drawing.Point(0, 148);
			this.SP_TMDb.Margin = new System.Windows.Forms.Padding(0);
			this.SP_TMDb.MaximumSize = new System.Drawing.Size(9999, 54);
			this.SP_TMDb.Name = "SP_TMDb";
			this.SP_TMDb.Size = new System.Drawing.Size(600, 54);
			this.SP_TMDb.TabIndex = 1;
			this.SP_TMDb.Text = "The Movie Database";
			// 
			// SP_ShowsCalendar
			// 
			this.SP_ShowsCalendar.Active = false;
			this.SP_ShowsCalendar.AutoHide = false;
			this.SP_ShowsCalendar.AutoSize = true;
			this.SP_ShowsCalendar.AutoSizeMode = System.Windows.Forms.AutoSizeMode.GrowAndShrink;
			this.SP_ShowsCalendar.Dock = System.Windows.Forms.DockStyle.Top;
			this.SP_ShowsCalendar.Flavor = null;
			this.SP_ShowsCalendar.Icon = global::TVShowsCalendar.Properties.Resources.Icon_22;
			this.SP_ShowsCalendar.Info = "";
			this.SP_ShowsCalendar.Location = new System.Drawing.Point(0, 0);
			this.SP_ShowsCalendar.Margin = new System.Windows.Forms.Padding(0);
			this.SP_ShowsCalendar.MaximumSize = new System.Drawing.Size(9999, 54);
			this.SP_ShowsCalendar.Name = "SP_ShowsCalendar";
			this.SP_ShowsCalendar.Size = new System.Drawing.Size(600, 54);
			this.SP_ShowsCalendar.TabIndex = 0;
			this.SP_ShowsCalendar.Text = "Shows Calendar";
			// 
			// L_TMDbInfo
			// 
			this.L_TMDbInfo.Dock = System.Windows.Forms.DockStyle.Top;
			this.L_TMDbInfo.Location = new System.Drawing.Point(7, 205);
			this.L_TMDbInfo.Margin = new System.Windows.Forms.Padding(7, 3, 7, 3);
			this.L_TMDbInfo.Name = "L_TMDbInfo";
			this.L_TMDbInfo.Size = new System.Drawing.Size(586, 76);
			this.L_TMDbInfo.TabIndex = 14;
			this.L_TMDbInfo.Text = resources.GetString("L_TMDbInfo.Text");
			// 
			// P_Tiles
			// 
			this.P_Tiles.Controls.Add(this.T_Changelog);
			this.P_Tiles.Controls.Add(this.T_Licence);
			this.P_Tiles.Controls.Add(this.T_Tips);
			this.P_Tiles.Controls.Add(this.T_AppInfo);
			this.P_Tiles.Dock = System.Windows.Forms.DockStyle.Fill;
			this.P_Tiles.Location = new System.Drawing.Point(0, 1);
			this.P_Tiles.Margin = new System.Windows.Forms.Padding(0);
			this.P_Tiles.Name = "P_Tiles";
			this.P_Tiles.Size = new System.Drawing.Size(175, 1407);
			this.P_Tiles.TabIndex = 35;
			// 
			// T_Changelog
			// 
			this.T_Changelog.Dock = System.Windows.Forms.DockStyle.Top;
			this.T_Changelog.DrawLeft = false;
			this.T_Changelog.Font = new System.Drawing.Font("Nirmala UI", 9.75F);
			this.T_Changelog.Image = global::TVShowsCalendar.Properties.Resources.ArrowRight;
			this.T_Changelog.Location = new System.Drawing.Point(0, 135);
			this.T_Changelog.Name = "T_Changelog";
			this.T_Changelog.Padding = new System.Windows.Forms.Padding(10);
			this.T_Changelog.Size = new System.Drawing.Size(175, 45);
			this.T_Changelog.TabIndex = 39;
			this.T_Changelog.TabStop = false;
			this.T_Changelog.Text = "Change-log";
			this.T_Changelog.Click += new System.EventHandler(this.T_Changelog_Click);
			// 
			// T_Licence
			// 
			this.T_Licence.Dock = System.Windows.Forms.DockStyle.Top;
			this.T_Licence.DrawLeft = false;
			this.T_Licence.Font = new System.Drawing.Font("Nirmala UI", 9.75F);
			this.T_Licence.Image = global::TVShowsCalendar.Properties.Resources.ArrowRight;
			this.T_Licence.Location = new System.Drawing.Point(0, 90);
			this.T_Licence.Name = "T_Licence";
			this.T_Licence.Padding = new System.Windows.Forms.Padding(10);
			this.T_Licence.Size = new System.Drawing.Size(175, 45);
			this.T_Licence.TabIndex = 40;
			this.T_Licence.TabStop = false;
			this.T_Licence.Text = "Licence Agreement";
			this.T_Licence.Visible = false;
			this.T_Licence.Click += new System.EventHandler(this.T_Licence_Click);
			// 
			// T_Tips
			// 
			this.T_Tips.Dock = System.Windows.Forms.DockStyle.Top;
			this.T_Tips.DrawLeft = false;
			this.T_Tips.Font = new System.Drawing.Font("Nirmala UI", 9.75F);
			this.T_Tips.Image = global::TVShowsCalendar.Properties.Resources.ArrowRight;
			this.T_Tips.Location = new System.Drawing.Point(0, 45);
			this.T_Tips.Name = "T_Tips";
			this.T_Tips.Padding = new System.Windows.Forms.Padding(10);
			this.T_Tips.Size = new System.Drawing.Size(175, 45);
			this.T_Tips.TabIndex = 38;
			this.T_Tips.TabStop = false;
			this.T_Tips.Text = "Tips & Tricks";
			this.T_Tips.Visible = false;
			this.T_Tips.Click += new System.EventHandler(this.T_Tips_Click);
			// 
			// T_AppInfo
			// 
			this.T_AppInfo.Dock = System.Windows.Forms.DockStyle.Top;
			this.T_AppInfo.DrawLeft = false;
			this.T_AppInfo.Font = new System.Drawing.Font("Nirmala UI", 9.75F);
			this.T_AppInfo.Image = global::TVShowsCalendar.Properties.Resources.ArrowRight;
			this.T_AppInfo.Location = new System.Drawing.Point(0, 0);
			this.T_AppInfo.Name = "T_AppInfo";
			this.T_AppInfo.Padding = new System.Windows.Forms.Padding(10);
			this.T_AppInfo.Selected = true;
			this.T_AppInfo.Size = new System.Drawing.Size(175, 45);
			this.T_AppInfo.TabIndex = 37;
			this.T_AppInfo.TabStop = false;
			this.T_AppInfo.Text = "App Info";
			this.T_AppInfo.Click += new System.EventHandler(this.T_AppInfo_Click);
			// 
			// P_Spacer
			// 
			this.P_Spacer.Dock = System.Windows.Forms.DockStyle.Fill;
			this.P_Spacer.Location = new System.Drawing.Point(175, 1);
			this.P_Spacer.Margin = new System.Windows.Forms.Padding(0);
			this.P_Spacer.Name = "P_Spacer";
			this.P_Spacer.Size = new System.Drawing.Size(1, 1407);
			this.P_Spacer.TabIndex = 36;
			this.P_Spacer.Paint += new System.Windows.Forms.PaintEventHandler(this.P_Spacer_Paint);
			// 
			// P_Spacer_2
			// 
			this.TLP_Main.SetColumnSpan(this.P_Spacer_2, 3);
			this.P_Spacer_2.Dock = System.Windows.Forms.DockStyle.Fill;
			this.P_Spacer_2.Location = new System.Drawing.Point(0, 0);
			this.P_Spacer_2.Margin = new System.Windows.Forms.Padding(0);
			this.P_Spacer_2.Name = "P_Spacer_2";
			this.P_Spacer_2.Size = new System.Drawing.Size(783, 1);
			this.P_Spacer_2.TabIndex = 37;
			this.P_Spacer_2.Paint += new System.Windows.Forms.PaintEventHandler(this.P_Spacer_2_Paint);
			// 
			// Content
			// 
			this.Content.AutoSize = true;
			this.Content.AutoSizeMode = System.Windows.Forms.AutoSizeMode.GrowAndShrink;
			this.Content.Dock = System.Windows.Forms.DockStyle.Top;
			this.Content.Location = new System.Drawing.Point(43, 54);
			this.Content.Margin = new System.Windows.Forms.Padding(3, 3, 0, 0);
			this.Content.Name = "Content";
			this.Content.Size = new System.Drawing.Size(557, 13);
			this.Content.TabIndex = 0;
			// 
			// PC_About
			// 
			this.AutoScaleDimensions = new System.Drawing.SizeF(96F, 96F);
			this.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Dpi;
			this.Controls.Add(this.TLP_Main);
			this.Name = "PC_About";
			this.Padding = new System.Windows.Forms.Padding(0, 30, 0, 0);
			this.Size = new System.Drawing.Size(783, 1438);
			this.Text = "About";
			this.Resize += new System.EventHandler(this.PC_About_Resize);
			this.Controls.SetChildIndex(this.TLP_Main, 0);
			this.TLP_Main.ResumeLayout(false);
			this.P_OptionContainer.ResumeLayout(false);
			this.P_OptionContainer.PerformLayout();
			this.P_Stuff.ResumeLayout(false);
			this.P_Stuff.PerformLayout();
			this.P_Changelog.ResumeLayout(false);
			this.panel1.ResumeLayout(false);
			((System.ComponentModel.ISupportInitialize)(this.B_ChangeLog)).EndInit();
			this.P_Tips.ResumeLayout(false);
			this.P_Tips.PerformLayout();
			this.P_AppInfo.ResumeLayout(false);
			this.P_AppInfo.PerformLayout();
			((System.ComponentModel.ISupportInitialize)(this.B_Icons8)).EndInit();
			((System.ComponentModel.ISupportInitialize)(this.B_TMDb)).EndInit();
			this.P_Tiles.ResumeLayout(false);
			this.ResumeLayout(false);
			this.PerformLayout();

		}

		#endregion

		private System.Windows.Forms.TableLayoutPanel TLP_Main;
		private System.Windows.Forms.Panel P_OptionContainer;
		private SlickControls.Controls.VerticalScroll verticalScroll1;
		private System.Windows.Forms.Panel P_Stuff;
		private System.Windows.Forms.Panel P_Tiles;
		private SlickControls.Controls.SlickTile T_Changelog;
		private SlickControls.Controls.SlickTile T_Tips;
		private SlickControls.Controls.SlickTile T_AppInfo;
		private SlickControls.Controls.DBPanel P_Spacer;
		private SlickControls.Controls.DBPanel P_Spacer_2;
		private System.Windows.Forms.TableLayoutPanel P_AppInfo;
		private SlickControls.Controls.SlickTile T_Licence;
		private System.Windows.Forms.Panel P_Changelog;
		private System.Windows.Forms.TableLayoutPanel P_Licence;
		private System.Windows.Forms.TableLayoutPanel P_Tips;
		private System.Windows.Forms.Panel panel1;
		private SlickControls.Controls.SlickButton B_ChangeLog;
		private UserControls.SectionPanel SP_TMDb;
		private UserControls.SectionPanel SP_ShowsCalendar;
		private UserControls.SectionPanel SP_Icons8;
		private UserControls.AutoSizeLabel L_TMDbInfo;
		private SlickControls.Controls.SlickButton B_Icons8;
		private SlickControls.Controls.SlickButton B_TMDb;
		private UserControls.AutoSizeLabel L_Icons8Info;
		private UserControls.AutoSizeLabel L_ShowsCalendarInfo;
		private System.Windows.Forms.FlowLayoutPanel Content;
		private UserControls.AutoSizeLabel autoSizeLabel1;
		private UserControls.SectionPanel sectionPanel1;
	}
}
