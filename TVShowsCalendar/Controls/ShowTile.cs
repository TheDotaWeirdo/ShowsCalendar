...
namespace ShowsCalendar
{
	public partial class ShowTile : PictureBox
	{
		...
		{
			Data.Mainform.PushPanel(null, new PC_ShowPage(LinkedShow));
		}
		...
		private void ShowTile_MouseClick(object sender, MouseEventArgs e)
		{
			...
			else if(new Rectangle(1, 0, Width - 2, (Width - 2) * 9 / 16).Contains(e.Location))
			{
				...
				{
					if ((LinkedShow.GetCurrentWatchEpisode()?.VidFiles.Any(x => x.Exists) ?? false) && new Rectangle(1 + (Width - 2) / 2, 0, (Width - 2) / 2, (Width - 2) * 9 / 16).Contains(e.Location))
		...
		private void ShowTile_Paint(object sender, PaintEventArgs e)
		{
			...
			{
				if (LinkedShow.GetCurrentWatchEpisode()?.VidFiles.Any(y => y.Exists) ?? false)
